...
function NavBar({ handleClick, darkMode }) {
  ...
  return (
    <nav className={`flex items-center ${section.padding} ${toggle ? 'fixed left-0 right-0 z-50 bg-[#ffffff] dark:bg-deep-blue' : ''}`}>
      {/* logo image */}
      {/* <div>
      ...
      <div className="flex items-center gap-3 ml-auto sm:gap-7">
        ...
        {!toggle &&  <FaBars className="text-2xl cursor-pointer sm:hidden text-deep-blue dark:text-gray " onClick={handleModal}/>}
        {/* rendering Times Icon with toggle condition */}
        {toggle && <FaTimes className="text-2xl cursor-pointer sm:hidden text-deep-blue dark:text-gray " onClick={handleModal}/>}
